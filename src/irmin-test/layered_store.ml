...
module Make_Layered (S : LAYERED_STORE) = struct
  ...
  let test_graph_and_history x () =
    let test repo =
      ...
      let* commit =
        fail_with_none (S.Commit.of_id repo kr2') "of_hash commit"
      in
      S.freeze repo ~max_lower:[ commit ] ~max_upper:[] >>= fun () ->
      let* t1' = P.Commit.find (h repo) kr1 in
  ...
  let test_gc x () =
    ...
    *)
    ...
    let test repo =
      ...
      check_val "c7" t7 t7';
      let* c7 =
        fail_with_none (S.Commit.of_id repo (S.Commit.id c7)) "of_hash commit"
      in
  ...
  let test_set x () =
    ...
    let check_parents = checks S.Id.t in
  ...
  let test_copy_in_upper x () =
    let test repo =
      ...
      (* Test that commit c1 and all its objects are preserved in upper after a
         freeze. *)
      let* c1 = fail_with_none (S.Commit.of_id repo kr1) "of_hash commit" in
      (* copy in upper too *)
      ...
      (* Test that commits c1 and c2 are preserved in upper during and after a
         freeze. *)
      let* c2 = fail_with_none (S.Commit.of_id repo kr2) "of_hash commit" in
      let* () =
        (* copy in upper too *)
  ...
  let test_find_during_freeze x () =
    ...
    let test repo =
      ...
      let add_and_find_commit ~hook v =
        ...
        let* () =
          (* copy in upper too *)
          S.PrivateLayer.freeze' ~hook ~max_lower:[ c ] repo
        in
  ...
  let test_add_during_freeze x () =
    let test repo =
      ...
      let add_and_find_commit ~hook t v =
        ...
        (* copy in upper too *)
        S.PrivateLayer.freeze' ~hook repo >>= fun () ->
        p >>= fun () ->
        find_commit t v () >>= fun () ->
  ...
  let test_add_again x () =
    let test repo =
      ...
      let* () =
        (* copy in upper too *)
        let* heads = S.Repo.heads repo in
        S.PrivateLayer.freeze' repo ~hook ~min_lower:heads ~max_lower:heads
      in
      p >>= fun () ->
      S.PrivateLayer.wait_for_freeze repo >>= fun () ->
